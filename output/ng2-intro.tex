...
\subsection{Class Decorators}\label{class-decorators}
...
selector: \texttt{string} value defining the css selector inputs:
\texttt{array\ of\ string} values defining the inputs to the component
outputs: \texttt{array\ of\ string} values defining the output of the
component properties: \texttt{array\ of\ string} values defining the
properties events: \texttt{array\ of\ string} values defining the events
host?: \{{[}key: string{]}: string\}, providers:
\texttt{array\ of\ objects} defining the providers for the component
exportAs: \texttt{string} value defining the exported value moduleId:
\texttt{string} value defining the module id viewProviders:
\texttt{array\ of\ objects} defining the providers for the view queries:
\{{[}key: string{]}: any\}, changeDetection:
\texttt{ChangeDetectionStrategy} object defining the strategy for
detecting changes:
...
template: \texttt{string} value for the template styleUrls:
\texttt{array\ of\ string} values defining url paths to css files
...
directives: \texttt{array} of directives used in the component pipes:
\texttt{array} of pipes used in the component encapsulation:
\texttt{ViewEncapsulation} value that defines template and style
encapsulation options: - \texttt{ViewEncapsulation.None}: means do not
provide any style encapsulation - \texttt{ViewEncapsulation.Emulated}:
No Shadow DOM but style encapsulation emulation using extra attributes
on the DOM (default method) - \texttt{ViewEncapsulation.Native}: means
provide native shadow DOM encapsulation and styles appear in component's
template inside the shadow root.
